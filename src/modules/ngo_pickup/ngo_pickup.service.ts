...
export class NGOPickupService {
  ...
  async findPendingPickupRequests(latLng: any, status: string): Promise<any> {
    ...
    const query = `select b.id as pickupId, a.name, a.email, b.price, b.prepared_time, b.expiry_time, b.number_of_meals FROM public."Users" as a Inner join public."PickupRequests" as b on b."userId" = a.id where a."lat" >=${lat_min} and a."lng" <= ${lat_max} and a."lng" >=${lng_min} and a."lng"<=${lng_max} and role = 'restaurant' and status='${status}'`;
